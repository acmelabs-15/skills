/**
 * spec-skill DESIGN schema validator (SPEC-008 TASK-015).
 *
 * Schema-only validator: parses a DESIGN note against DesignNoteSchema (via the
...
export async function validateDesignSchema(argv: string[], projectRoot: string): Promise<number> {
  const userPath = argv[0];
  if (userPath === undefined || userPath.length === 0) {
    console.error("usage: validate-design-schema <note-path>");
    return 2;
  }
...
if (import.meta.main) {
  validateDesignSchema(Bun.argv.slice(2), process.cwd()).then((code) => process.exit(code));
}
