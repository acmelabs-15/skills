...
This is not licence to remove a legitimate second enforcement POINT. The exemplar to preserve is `f8Map`: one predicate consumed by both map primitives, so the F-8 rule has a single implementation and two call sites. Recorded as a known finding: for the map rule that duality is currently predicate-only. The invariant runs as a parse-time schema refinement and has no runtime-callable form, so a caller holding an already-parsed map cannot re-assert it without re-parsing. Hoisting the predicate to a directly callable validator alongside its schema use is a follow-up rather than a defect — and until that lands, the claim-validator pattern, where a schema refinement and a standalone validator function genuinely do coexist, must not be cited as though it already described this rule.
...
- The F-8 char-identity guarantee is bounded, not universal: it holds over the preserved content slice, not over whole destination files. Retention-bearing plans do not round-trip — a retained range is proven by the coverage check and written nowhere, so the source cannot be reconstructed from the shards alone. Scaffolded no-retention plans DO round-trip, conditionally: recovery requires the composition plan to restate a byte-identical scaffold. Two sources can satisfy that condition, and they differ in durability. The distribution plan is the PRIMARY: it is a file on disk carrying each cluster's scaffold verbatim, and the executor reads it without ever rewriting, moving, or deleting it, so it persists after the run and can be copied from directly. The decompose audit log is SECONDARY and caller-captured: it records the exact scaffold applied together with its provenance, its byte count, and the hash of the content slice, but it is emitted as JSON-lines on stdout with no durable sink, so it survives only if the caller redirects or captures that stream. Recovery therefore does not require re-authoring provenance by hand, provided the plan was retained — or the audit was captured at run time. Retained content is additionally not guaranteed ABSENT from destinations — nothing validates that a destination's scaffold epilogue does not reproduce a retained observation or relation — so retention is a bound on reversibility, not redaction
...
**2026-07-26 (round-2 micro-corrections)**: Six converged corrections from the round-2 register, recorded here per the self-documentation norm the preceding entry established. The recovery sentence in Consequences now distinguishes two sources by durability — the distribution plan is primary and persists on disk, the audit log is secondary and survives only if the caller captures stdout. The guard-census Confirmation item now carries an explicit guard-set scope, so the census cannot be read as covering the claim validators. The one-implementation carve-out exemplar is re-pointed at `f8Map`, with the predicate-only status of the map rule recorded as a known finding and a hoist to a runtime-callable form logged as follow-up; the previous wording described the claim-validator pattern as though it already applied here, which it does not. The serializer-stability obligation now names the three test files a js-yaml bump must re-run, and names the one that does not substitute for them. The mechanical dead-path check is now two counts rather than one, call sites and implementations, with the doubly-declared plan size guard as the live case a call-site census alone would pass. And the path-containment mitigation now states its fail-open precondition on `SKILLS_DOCS_ROOT`, which is set nowhere outside test setup. No locked decision changed in this pass: every correction narrows, qualifies, or adds evidence to an existing claim rather than reversing one.
