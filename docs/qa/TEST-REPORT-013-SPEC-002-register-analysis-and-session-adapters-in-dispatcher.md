---
title: 'TEST-REPORT-013-SPEC-002: Register ANALYSIS and SESSION Adapters in Dispatcher'
type: test-report
permalink: qa/test-report-013-spec-002-register-analysis-and-session-adapters-in-dispatcher-1
status: DONE
tags:
- test-report
- spec-002
...
# TEST-REPORT-013-SPEC-002: Register ANALYSIS and SESSION Adapters in Dispatcher
...
1. The session schema file `schemas/distribution/session.plan.schema.ts` carries a different `cross_source_updates` shape than DESIGN-002 prescribes — this is recorded under TEST-REPORT-012 (TASK-003 gap), not against TASK-004 which only requires the union extension to validate the actual shape.
