...
- [x] File `shared/composition/tests/integration/qa-vs-task-dod.test.ts` exists
- [x] QA-vs-TASK-DoD test fails on a fixture pair where QA claims PASS but the linked TASK DoD line is `[ ]`
...
- [x] Honors ADR-005 D-3 Phase 3 critic P1.2 / P1.3 verbatim: parse-mutate-validate-render full path; cross-note SPEC-vs-TASK consistency; QA-vs-TASK-DoD cross-validation
...
- [x] Honors REQ-007 AC-3: QA-vs-TASK-DoD cross-validation test exists and fails on intentional drift
...
- [technique] Cross-note tests synthesize fixture pairs (SPEC root + child TASK; QA + TASK) and assert the consistency rule across the pair rather than within a single note #cross-note-pattern
