...
- [ ] `hooks/scripts/pre-commit-validate.ts` exists; binds Layer 3 matcher and `if` filter
- [ ] pre-commit-validate calls `readStagedBrainNotes(repoRoot)` and dispatches each to the validator
- [ ] pre-commit-validate emits `permissionDecision: "deny"` with reason naming every failing staged note when ANY fail
- [ ] `hooks/scripts/pre-push-validate.ts` exists; binds Layer 4 matcher and `if` filter
- [ ] pre-push-validate parses `git push [-u] [remote] [branch]` argument forms to identify the upstream target
- [ ] pre-push-validate calls `readPushDiffBrainNotes(repoRoot, remote, branch)` and dispatches each note
- [ ] pre-push-validate emits `permissionDecision: "deny"` with reason naming every failing pushed note when ANY fail
- [ ] `hooks/scripts/pre-pr-create-validate.ts` exists; binds Layer 5 matcher and `if` filter
- [ ] pre-pr-create-validate parses `gh pr create [--base BRANCH]` argument forms; defaults to `origin/HEAD` when `--base` is absent
- [ ] pre-pr-create-validate calls `readPrDiffBrainNotes(repoRoot, baseBranch)` and dispatches each note
- [ ] pre-pr-create-validate emits `permissionDecision: "deny"` with reason naming every failing PR-diff note when ANY fail
- [ ] All three handlers validate path containment for `repoRoot` and any path passed to git subprocesses
- [ ] All three handlers wrap validator invocation in try/catch and exit non-zero with structured stderr error on exception (fail-open)
- [ ] Unit tests cover deny path (one failing staged/pushed/PR note), allow path (clean set), argument parsing edge cases (no upstream, no `--base`), and traversal rejection
- [ ] Per-commit hook latency stays within ~500ms-2s for typical 5-10 file commit
- [ ] biome lint passes
- [ ] `bun tsc --noEmit` passes

## ADR Compliance

- [ ] Honors [[ADR-005: Protocol Hardening Wave 2 Architecture]] D-8 Layer 3-5 declarations (`Bash` matcher with the three `if` filters verbatim)
- [ ] Honors Phase 3 security P1 — argument parsing rejects traversal in any path argument before invoking git subprocesses
- [ ] Honors hybrid failure semantics — commit/push/PR gates deny on ANY failing note (the more conservative per-batch semantics from ADR-005)
