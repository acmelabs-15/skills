...
- [ ] File `shared/composition/src/schemas/analysis-note.ts` exists and exports `AnalysisNoteSchema` plus `type AnalysisNote`
- [ ] Frontmatter sub-schema enforces title regex, type literal `analysis`, status enum, permalink regex, tags 2-5
- [ ] superRefine rejects `status: ACCEPTED` plus presence of `## Open Questions` section in body sections list with a message naming the forbidden section
- [ ] Schema accepts `status: ACCEPTED` plus `## Open Questions` ABSENT
- [ ] Schema accepts `status: DRAFT` plus `## Open Questions` PRESENT (rule only fires at ACCEPTED)
...
- [ ] Unit tests cover: valid DRAFT, valid ACCEPTED without Open Questions, ACCEPTED plus Open Questions rejection, DRAFT plus Open Questions accepted, frontmatter shape failures
- [ ] `bun test shared/composition/tests/schemas/analysis-note.test.ts` passes with at least 6 cases green
- [ ] `biome check` passes on the new file
- [ ] `tsc --noEmit` passes
- [ ] `shared/composition/src/schemas/index.ts` re-exports `AnalysisNoteSchema` and `AnalysisNote`

## ADR Compliance

- [ ] Honors [[ADR-005: Protocol Hardening Wave 2 Architecture]] D-2 (flat directory placement)
- [ ] Honors [[ADR-005: Protocol Hardening Wave 2 Architecture]] D-5 (P1 ANALYSIS coverage closes Wave 7 exploit)
