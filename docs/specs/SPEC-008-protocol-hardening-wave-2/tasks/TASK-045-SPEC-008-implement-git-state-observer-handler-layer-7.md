...
- [x] `hooks/scripts/git-state-observer.ts` exists; binds Layer 7 matcher
- [x] Handler resolves repo root and reads `HEAD` SHA via `git rev-parse HEAD`
- [x] Handler enumerates `docs/**/*.md` files touched by the new commit via `git diff-tree`
- [x] Handler reads each touched file's post-commit content (current on-disk state) and dispatches through `dispatchValidator`
- [x] Handler aggregates PASS/FAIL counts and the failing-file list into a summary string
- [x] Handler emits `{ hookSpecificOutput: { hookEventName: "FileChanged", additionalContext: "Post-commit state: commit <sha> landed; full graph validation: <summary>" } }`
- [x] Handler does NOT emit any `permissionDecision` or `decision` field; Layer 7 is observe-only
- [x] Handler wraps validator invocation in try/catch and emits a degraded `additionalContext` ("Post-commit state: validation infrastructure error; manual inspection required") on exception (fail-open observe-only)
- [x] Handler does NOT fire on external editor edits that do not touch `.git/HEAD`/`.git/index`/`.git/logs/HEAD` (verified by integration test)
- [x] Unit tests cover happy path, all-passing summary, mixed-pass-fail summary, infrastructure error
- [x] biome lint passes
- [x] `bun tsc --noEmit` passes

## ADR Compliance

- [x] Honors [[ADR-005: Protocol Hardening Wave 2 Architecture]] D-8 Layer 7 declaration (FileChanged matcher with literal filenames; observe-only)
- [x] Honors [[REQ-012-SPEC-008: Stop Backstop and File Changed Observability]] AC — handler emits additionalContext only; external editor edits out of scope
...
- relates_to [[QA-083-SPEC-008: Validation Report for TASK-045 git-state-observer Handler Layer 7]]
