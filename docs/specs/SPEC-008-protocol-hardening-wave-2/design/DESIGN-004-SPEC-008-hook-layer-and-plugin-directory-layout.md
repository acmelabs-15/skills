...
**Layered-severity verdict mapping** (amended 2026-05-24 SESSION-2026-05-23_02 Event 114, user-approved; resolves the REQ-011 AC#6/AC#7 tension). `dispatchValidator` returns a 3-way verdict (`deny` = claim-validator failure; `allow-with-warning` = non-claim hygiene/schema issue; `allow` = clean). Each handler maps the verdict by its layer class, so severity matches the moment: PER-WRITE gates (Layer 1 local, Layer 2 MCP) map `allow-with-warning` → ALLOW (surface `additionalContext`; the write proceeds, so a hygiene-imperfect note stays editable and can be fixed incrementally — no self-lock). BOUNDARY + BACKSTOP gates (Layer 3 pre-commit, Layer 4 pre-push, Layer 5 pre-PR, Layer 6 Stop) map `allow-with-warning` → DENY (full conformance required — nothing non-conformant, claim OR hygiene, enters history / is shared / survives turn-end). Layer 7 observes only. Net effect: both claim-lies and authoring hygiene are caught and forced-fixed, but blocking happens only at layers where it does not prevent the fix.
...
Layer 6 (`Stop`) re-runs the per-note validation against every `docs/**` file modified during the turn by enumerating uncommitted working-tree changes via `git status --porcelain` (amended 2026-05-24 SESSION-2026-05-23_02 Event 105, user-approved: git-status enumeration replaces transcript-walk per REQ-012 AC1 — transcript-parsing misses Brain-MCP edits and mtime-scanning misses reverts; git-status catches tool-mediated edits regardless of the Edit/Write vs MCP path). If any note fails, the handler emits `{ decision: "block", reason: "..." }` to block turn completion. The Stop hook is the defense-in-depth backstop for the matcher-gap risk: if a `PreToolUse` matcher fails to fire on `mcp__plugin_brain_brain__*` (Layer 2) or any other path, the turn end still runs the validators before the agent yields control.
