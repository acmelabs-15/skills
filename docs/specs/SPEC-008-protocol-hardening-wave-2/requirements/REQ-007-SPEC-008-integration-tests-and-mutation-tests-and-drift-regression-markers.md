...
THE SYSTEM SHALL include a dedicated integration-test layer that exercises the full parse-then-mutate-then-validate-then-render path on representative fixtures, AND cross-note consistency tests that detect SPEC-vs-child-TASK status mismatches, AND cross-note QA-vs-TASK-DoD validation
...
- [x] GIVEN the integration-test file `shared/composition/tests/integration/qa-vs-task-dod.test.ts` WHEN it runs THEN it asserts the rule: every QA row marked PASS MUST correspond to a TASK DoD line marked `[x]`; the test fails when given a fixture pair where a QA note claims PASS but the linked TASK DoD line is still `[ ]`
...
| `shared/composition/tests/integration/qa-vs-task-dod.test.ts` | NEW | QA-vs-TASK DoD cross-validation |
...
- [constraint] Cross-note consistency tests require fixture pairs (SPEC root + child TASK; QA note + TASK); fixture-pair construction adds to test-authoring cost but is bounded to ~5 pairs #fixture-pairs
...
- [outcome] Audit E's three top-priority integration gaps (full pipeline, SPEC-vs-TASK, QA-vs-TASK) become regression-locked; the protocol's end-to-end correctness moves from prose to runtime #end-to-end-contract
