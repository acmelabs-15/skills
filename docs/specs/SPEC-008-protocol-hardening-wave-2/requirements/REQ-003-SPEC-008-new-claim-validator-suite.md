...
- [ ] GIVEN a PLAN note with status DONE and one part whose substatus is `IN_PROGRESS` WHEN `validatePlanDoneClaim(planNote)` is called THEN it returns `{ ok: false }` naming the non-terminal part
- [ ] GIVEN a PLAN note with status DONE and every part in a terminal substatus (DONE, DEFERRED, or ABANDONED) WHEN `validatePlanDoneClaim(planNote)` is called THEN it returns `{ ok: true, unsatisfied: [] }`
