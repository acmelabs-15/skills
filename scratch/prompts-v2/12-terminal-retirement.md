...
- The prompt-3 export program is dissolved, and it now has **no survivors**. `feedback_claude_code_markdown_first.md` was its one ratified survivor, slated for migration into the skill-authoring style guide; the owner **deleted the rule outright** on 2026-07-28 instead (R-33: *"That doesn't mean every rule is going to survive… In this case, I think we just delete this rule"*). Prompt 4's append step is dropped, so there is **no append to verify and no read-back gate** — see step 6. R-33 also sets a standing frame for this prompt: migration is not the default, and a rule may be deleted, or survive re-authored rather than verbatim.
...
6. **`feedback_claude_code_markdown_first.md` → DELETED [DECIDED — R-33, superseding R-27/R-28]:** delete it through the `rm` gate and remove its `MEMORY.md` index line. **No append to verify, no read-back, no stop condition** — the owner ruled the rule deleted rather than migrated on 2026-07-28, and the destination file (`authoring-style.md`) no longer exists in any case, having dissolved into the successor plugin's ten references. The previous instruction to halt when the append was absent would have blocked on a condition that can never be satisfied. Record the reversal in the audit so a later reader knows why the programme's "one ratified survivor" left no trace.
...
| Markdown-first memory: **DELETED outright** here — no append, no read-back, no stop condition | R-33 (superseding R-27/R-28) |
| Migration is not the default: a rule may be deleted, or survive re-authored rather than verbatim | R-33 |
...
- [ ] `feedback_claude_code_markdown_first.md` was deleted outright through the `rm` gate with its index line removed, and the R-33 reversal is recorded in the audit — no append was sought, no read-back performed, no destination invented.
