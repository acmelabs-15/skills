...
## The authority rule  [DECIDED 2026-07-27; authority substituted 2026-07-28 per R-32]

**`skill-creator` (the `skill-creator@ACMElabs` plugin, `/Users/peter.kloss/Dev/ACMElabs/skill-creator/skills/skill-creator/`) is the authority for skill-authoring mechanics.** Load it **whole** before starting and let it drive the refactor lifecycle end to end — its loop, its eval workflow, its validators, its own sign-off gates, at its own pace. This prompt deliberately does **not** restate the skill's internals; its ten references carry the operational traps. Where anything in this prompt appears to conflict with `skill-creator` about *how* to author or test a skill, `skill-creator` wins — say so in the report when it happens.

**The predecessor `create-skill` was deleted** (owner, 2026-07-28: *"we removed the create-skill skill and replaced it with the skill-creator skill which should b used instead"*). Substitution with substantive deltas, not a rename. Four bind on this prompt's own steps:

- **Descriptions invert.** The old guide taught *"deliberately pushy"*; the successor makes that a named defect (`description-writing.md:57`, criterion 4) and measures the cost — with a pushy neighbour installed, a shipped description lost **all 21** of its own true positives (85.7% → 0.0%). Replacement rule: matchable on the artifact produced, not the topic; a deliverable clause plus at least one same-domain "do not use when". Governs step 8.
- **Body size is a dual limit** — under 500 lines **and** under 5,000 tokens, both measured, both reported (`progressive-disclosure.md:13`). The ~450-line ceiling this prompt calls its one hard gate no longer exists as stated [owner-ruled 2026-07-28].
- **Directory placement is decided by the verb SKILL.md uses, not file type** (`progressive-disclosure.md:41-61`): a `scripts/` file is never read, an `examples/` file is never run, and *"do not create a directory for one file"* — a lone reference belongs at the skill root.
- **`skill-reviewer` resolves**: it ships in-plugin as `skill-creator:skill-reviewer`, so the verify-before-gating caveat is satisfied and no fallback is needed. `skill-creator@claude-plugins-official` stays `false` (`settings.json:245`/`:257`); `skill-creator@ACMElabs` is `true` (`:269`) and is the live path.

The predecessor survives read-only in the prompt-1 snapshot at `skill-snapshot/create-skill/` — cite it as history, never as authority.
...
- **P4-5 — The full `skill-creator` lifecycle runs for every skill touched, under the skill's own procedure.** [DECIDED — D-2 plus the authority rule above.] D-2 verbatim: *"Run the **full create-skill lifecycle including the empirical stages 7–9** (evals, benchmark, `run_loop` description optimization). Not validate-only."* The successor agrees — `authoring-checklist.md:5`: *"Treat a clean checklist as permission to start evaluating, never as evidence of quality."* A static audit does not discharge P4-5.
  - **Repo fact (1) — the snapshot.** Prompt 1 already took the week-0 snapshot to a dated off-tree workspace. Find it, confirm it covers all three skills, record the path. **If it does not exist, stop and tell Peter** — a skill edited without its before-state is permanently unmeasurable. (Verified 2026-07-28: `~/Desktop/brain-consolidation-baseline-2026-07-27/`, 34 MB / 838 files / 62 skills; all three at `skill-snapshot/<name>/`, byte-matching the live files.)
  - **Repo fact (2) — the benchmark needs a graph these skills may safely mutate.** All three write only through the composition CLIs, which take `--docs-root` as a path argument and `--project` to pin the graph, so a throwaway Brain project is a valid eval target and the live graph is never at risk. Built and verified 2026-07-28: project **`kg-fixture`** at `~/Desktop/kg-fixture/docs` (off-tree, not a git repo), 10 real notes copied from this project's graph — ADR-002 (982 lines, `### D-N` seams) as the split candidate; CRIT-002, CRIT-005, ANALYSIS-006 and PLAN-002 as inbound referrers; three thin QA notes as merge candidates; 11 naturally-dangling outbound edges. Synced with `brain sync --project kg-fixture --force`; `--references ADR-002` returns 4 inbound edges and *"Complete: every note in the project was accounted for"*; disk-vs-index parity is 10 = 10, which is the funnel's own blind-spot gate. **Pass `--project kg-fixture` on every command** [owner-instructed] — `--project` is optional everywhere, and omitting it resolves by working-directory match, which from this repo reaches the live graph.
  - **Prerequisite trap, measured.** `brain projects create` writes the project entry as a bare path string where basic-memory requires a five-field object, and basic-memory then **refuses to load the config at all** — every project goes dark, not just the new one. Use `basic-memory project add <name> <path>`, which owns the schema. Back up `~/.basic-memory/config.json` and `~/.config/brain/config.json` first. Report the bug; never route around it silently.
  - **Do not run markdown auto-fix over these prompt files.** Measured 2026-07-28 on this very sweep: `markdownlint-cli2 --fix` renumbered an ordered list (4-16 → 1-12) and stripped underscores out of `feedback_*` identifiers inside nested bullets, corrupting both. Diff-review any multi-file auto-fix before committing, or skip it here entirely.
...
- The style authority is `skill-creator`'s reference set — hand the files themselves to every agent rather than paraphrasing them. The predecessor's single `authoring-style.md` (294 lines) dissolved into ten references (~1,670 lines). Four bind here: `progressive-disclosure.md` (dual size limit, verb-decides-placement, one-place rule), `description-writing.md` (the four description criteria, measured), `frontmatter.md` (six standard fields, fail-open extensions, `version:` under `metadata:`), `authoring-checklist.md` (the 22-box pre-flight). Every old claim's new home was verified 2026-07-28; the **never-both rule survived**, restated harder at `progressive-disclosure.md:21` — *"If something is in SKILL.md and in a reference file, one of them will drift and the agent will read both"* — plus a checklist box at `authoring-checklist.md:47`.
...
- `skill-reviewer` is an external plugin agent, and both `skill-creator@claude-plugins-official` and `plugin-eval@claude-code-workflows` are `false` in `settings.json` — **verify `skill-reviewer` resolves before treating it as a gate**; otherwise use `skill-creator`'s inline fresh-eyes fallback and say which path ran.
...
1. **Load `skill-creator`; find the prompt-1 snapshot** (P4-5); run the lifecycle under the skill's own procedure and gates for all three skills. The skill's own sign-offs (e.g. eval-prompt approval, if its empirical stages apply here) are additional human rounds and belong to it, not to this prompt.
...
3. **Write the spines and references**, honoring the W-1 answers. Budgets [DECIDED 2026-07-27: **targets, not gates**]: aim ~195 (`decompose`) / ~150 (`recompose`) / ~130 (`defrag`); overshoot carries a stated reason in the report. The hard ceiling is `skill-creator`'s dual limit — under 500 lines **and** under 5,000 tokens, both measured and both reported [owner-ruled 2026-07-28, superseding the ~450-line figure]. Note its one-file rule: a skill that ends up with a single reference keeps that file at the skill root, not in a `references/` directory of one. Shared references [DECIDED as a leaning, not locked — *"happy to lean towards it… not something I want to lock in"*]: start from one shared impact-manifest reference (~80% common mechanics) plus ~35-line per-direction riders per the divergence facts above; `skill-creator`'s judgment plus measurement may override the cut — report the decision, don't ask.
...
8. **Frontmatter.** Remove `triggers:` from `defrag` and `ingest`, folding every trigger phrase into `description` — mechanically forced, not a preference: the loader silently ignores the key AND `skill-creator`'s validator hard-fails on it. Measured 2026-07-28: `bun scripts/quick-validate.ts skills/skills/defrag --extended` returns *"Unexpected key `triggers` in SKILL.md frontmatter"* and **Skill is invalid**, so the lifecycle cannot pass while it stays; `decompose` passes clean as a control. Rewrite both descriptions to the successor's rule, not the predecessor's — **capability-first** (the `"This skill should be used when…"` opener is rejected at `authoring-checklist.md:20` for burning the highest-signal tokens on a constant string), a deliverable clause, at least one same-domain "do not use when", and no universal-quantifier pushiness. Current lengths 436 (`defrag`) and 387 (`ingest`), far under the 1,024 cap. Do not add `version:` — `frontmatter.md:72` rules it non-conformant and routes it to `metadata:`, which **resolves** the predecessor's self-contradiction, so there is no upstream defect left to flag. Report the census: `review` **1,056** chars (over the cap, truncating live today), `spec` 990, `build` 968, and **eight of eleven** plugin skills opening with the rejected boilerplate — `review`, `spec`, `build` are **not yours to edit here**; measure, name the consequence, recommend.
...
10. ~~The markdown-first append.~~ **DROPPED [owner-ruled 2026-07-28, R-33].** The rule is deleted rather than migrated: *"That doesn't mean every rule is going to survive… In this case, I think we just delete this rule."* Its destination file no longer exists, and the owner declined to re-home it. `feedback_claude_code_markdown_first.md` is deleted outright by prompt 12 — no append, no read-back verification, no stop condition. Supersedes R-27/R-28's placement of the append here; the corresponding `## Done means` box is void.
...
| Decision | Source |
|---|---|
| skill-creator drives; this prompt supplies only repo facts, decisions, and Peter's open questions | owner, 2026-07-27 |
| `create-skill` is retired — deleted, replaced by the `skill-creator@ACMElabs` plugin; substantive deltas, not a rename | owner, 2026-07-28 (R-32) |
| Full lifecycle + independent-evaluation mandate; **"not validate-only"** | D-2 |
| Budgets are targets, not gates; the hard ceiling is the dual limit — under 500 lines AND under 5,000 tokens | owner, 2026-07-27; ceiling superseded 2026-07-28 (R-32) |
| Shared-reference design is a leaning starting design, `skill-creator` may override with measurement | owner, 2026-07-27 |
| The benchmark runs against the `kg-fixture` throwaway graph, `--project kg-fixture` on every command | owner-instructed, 2026-07-28 |
| The ACMElabs `defrag` is the only `defrag` (cleanup shape = W-2) | D-14 |
| `triggers:` removed from `defrag`+`ingest`, phrases folded into `description` | forced by skill-creator's own validator |
| ASK-STANDARD (AskUserQuestion usage standard) is developed in prompt 7's interview, not here | owner, 2026-07-27 |
| The markdown-first rule is **deleted, not migrated** — Step 10 dropped, its Done-means box void | owner, 2026-07-28 (R-33, superseding R-27/R-28) |
| Migration is not the default: a rule may be deleted, or survive re-authored rather than verbatim | owner, 2026-07-28 (R-33) |
...
- [ ] `skill-creator` loaded **whole** and drove the lifecycle; its own gates were respected; the prompt-1 snapshot was found and covers all three skills (or the run stopped).
- [ ] D-2's "not validate-only" satisfied: the empirical stages ran against `kg-fixture` with `--project kg-fixture` on every command, or the reason they could not is stated and owner-approved.
...
- [ ] `triggers:` gone from both skills with every phrase preserved in `description`; both descriptions rewritten capability-first with a deliverable clause and a same-domain exclusion, and no universal-quantifier pushiness; `bun scripts/quick-validate.ts <dir> --extended` passes for every skill touched; the description census is reported including `review`'s 1,056-char overrun and the eight skills carrying the rejected boilerplate opener.
- [ ] Every spine reports **both** measurements against the dual limit — line count and token count.
- [ ] Ranked findings delivered per skill; nothing applied without approval.
- [ ] ~~`authoring-style.md` appended.~~ Void per R-33 — the rule is deleted, not migrated; confirm no append was made and no destination was invented.
