...
- **`skill-creator`** (the `skill-creator@ACMElabs` plugin, `/Users/peter.kloss/Dev/ACMElabs/skill-creator/skills/skill-creator/`) owns skill-authoring and evaluation mechanics for every skill this prompt touches — its loop, its validators, its own sign-off gates, at its own pace. Load it whole before starting. **Its predecessor `create-skill` was deleted 2026-07-28 (R-32)**: descriptions are now capability-first with a deliverable clause and a same-domain exclusion (pushiness is a measured defect); body size is a dual limit of under 500 lines AND under 5,000 tokens, both reported; and the validator is `bun scripts/quick-validate.ts <dir> --extended`. Where this prompt appears to conflict with it about *how* to author or test a skill, `skill-creator` wins — say so in the report when it happens.
...
1. **Load `skill-creator`; find the prompt-1 snapshot** (P4-5). Deliver Step 0's map; wait for the go.
...
10. **Independent evaluation (P4-4/D-2)** over the twelve `spec` files and whatever this prompt touched in `research`/`decisions`, cold-read, beyond the brief. Findings are the options, ≤4 per call, consecutive calls, multiselect = apply; nothing unselected is applied. Then close out the `skill-creator` lifecycle per P4-5, numbers with their measurement basis (E-4).
...
| Independent evaluation + full skill-creator lifecycle, **not validate-only** | D-2 (authority substituted per R-32) |
...
- [ ] `skill-creator` loaded and drove the lifecycle; the prompt-1 snapshot found and covering (or the run stopped); its validators pass for every skill touched, no benchmark regression.
